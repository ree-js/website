...
<Alert text="Although you can consume the `isDevMode` argument in your function, copiers are only ran during build mode, and not in dev mode." />
...
<Alert text="Install the npm version of million and link it for the server side of import maps. Use URLs for browser side import maps. The server side is known to leverage npm hacks and methods which has a higher chance of not properly working." />
